...
export default function Format03({ data }: { data: Format03Data }) {
  const weeks = Array.from(new Set(data.events.map((e) => e.week))).sort(
    (a, b) => a - b,
  );
    ...
    >
            ...
            return DAYS.map((day, dayIdx) => {
              ...
              return (
                <tr
                  key={`${week}-${day}`}
                  style={{
                    backgroundColor: isSunday ? "#BDD7EE" : "transparent",
                  }}
                >
                  {dayIdx === 0 && (
                    <td
                  ...
                  <td style={{ border: "1px solid black", padding: "3pt 5pt" }}>
                    {day}
                  </td>
                  <td
                    style={{
      ...
      >
        <div style={{ maxWidth: "68%", fontSize: "8.5pt" }}>
          <strong>Note</strong> All the Co-Curricular (Including Curriculum
          gap), Extra Curricular activities for odd &amp; even Semester are to
          be planned at the commencement the academic year in line with the BTE
          academic calendar of events.
        </div>
        <div
          style={{ fontSize: "9.5pt", fontWeight: "bold", textAlign: "right" }}
        >
